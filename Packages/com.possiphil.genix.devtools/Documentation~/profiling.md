...
Use profiling to explain one run, not as the authoritative total for performance comparisons. Instrumentation adds work. For controlled comparisons, use the Benchmark window's Primary measurements and keep scene, object count, style, targets, seed policy, Unity version, and cache condition fixed.
