...
**Tools > Genix > Assets** and choose **Set Up Starter Content**. Genix creates an editable SFS room,
...
uses a new seed for each run. Enable **Use Seed** only when a reproducible comparison is required.
...
2. Keep the shared interface mode on **Basic** for the first run.
...
Switch to **Advanced** when the scene needs a non-default surface source, target or support distribution, relative placement, deterministic seed, search-budget tuning, or detailed diagnostics. The mode is shared by the designer windows and never changes stored settings.
