...
namespace Genix.Editor.Generation
{
    ...
    public static class GenerationWorkflow
    {
        ...
        {
            bool shouldProfile = GenerationInstrumentation.IsEnabled;
            ...
            IGenerationProfiler profiler = InitializeProfiler(
                GenerationInstrumentation.CreateProfiler(),
                context,
                request.StyleName,
                dryRun: false,
            ...
            try
            {
                ...
                ManagedRuntimeSnapshot runtimeBefore = profiler.IsEnabled
                ...
                ManagedRuntimeSnapshot runtimeAfter = profiler.IsEnabled
                    ? CaptureManagedRuntimeSnapshot()
                    : default;
                float planningMilliseconds = StopAndReadMilliseconds(planningStopwatch);
                RecordPlanningTime(profiler, planningMilliseconds);
                RecordManagedRuntimeStats(profiler, runtimeBefore, runtimeAfter);
                RecordProfilePlacedCount(profiler, outcome.PlacedCount);
                ...
                if (!outcome.ShouldApply)
                {
                    ...
                    RecordProfileStopReason(profiler, outcome.Message);
                ...
                if (!SceneGenerationService.Apply(context.Plan, generatedParent, out string applyError))
                {
                    ...
                    RecordProfileStopReason(profiler, applyError);
                    DiagnosticsStore.SetLast(recorder.Diagnostics);
                    SceneGenerationService.RemoveEmptyParent(generatedParent, parentExisted);
                    Debug.LogWarning(applyError);
                    return false;
            ...
            {
                FinishProfile(profiler, totalStopwatch);
            }
        }
            ...
            ManagedRuntimeSnapshot after)
        {
            ...
            profiler.Profile.RecordManagedRuntimeStats(
        ...
        private static void RecordProfilePlacedCount(IGenerationProfiler profiler, int count)
        {
            if (profiler is { IsEnabled: true })
                profiler.Profile.PlacedObjectCount = Mathf.Max(0, count);
        }

        private static void RecordProfileStopReason(IGenerationProfiler profiler, string stopReason)
        {
            if (profiler is { IsEnabled: true })
                profiler.Profile.StopReason = stopReason ?? string.Empty;
        }
            ...
            Stopwatch totalStopwatch)
        {
            ...
            GenerationInstrumentation.Store(profiler);
        }
