...
Open **Tools > Genix > Performance Benchmarks** to configure and run unattended, scene-based benchmark campaigns. Benchmark suites are project assets, so the exact scenes, target areas, generation settings, object counts, cache conditions, and deterministic seeds can be reviewed and versioned.
...
2. Use **Add Evaluation Scenes** to import scenes under `Evaluation/Scenes/Performance` and `Evaluation/Scenes/RealWorld`, or add scenarios manually.
